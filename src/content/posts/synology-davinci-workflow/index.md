...
除非你的 NAS 和电脑之间有极其稳定的 5Gbps 甚至万兆（10GbE）网络连接，否则博主强烈建议在项目中启用代理剪辑，并强制将渲染缓存保存在本地固态硬盘。

*代理剪辑的原理很简单：达芬奇会在后台把 NAS 上极难解码、码率极高的高画质原片，转码成体积小、极易解码的低画质版本（如 DNxHR LB）。剪辑时看低画质代理，丝滑无比；最终导出时，达芬奇会自动调取 NAS 上的原片进行高画质渲染。*