...
1. 在你电脑本地的达芬奇视频项目库（如博主的 `E:\视频工程`）新建 `ProxyMedia` 、`CacheClip` 和 `.gallery` 文件夹，分别保存代理、缓存、画廊静帧。有就不用再创建了。
2. 在达芬奇中打开新创建的项目，点击右下角齿轮进入 `项目设置  -> 主设置`。向下滚动找到 **工作文件夹**。将“代理生成位置”“缓存文件位置”和“画廊静帧位置”浏览指向你刚才在 E 盘新建的本地目录。![img_1781953212162.png](./img_1781953212162.png)
3. 在顶部菜单栏点击`播放 -> 代理的处理方式 -> 勾选 优先使用代理`。![img_1781953290850.png](./img_1781953290850.png)
4. 导入 NAS 视频后，在媒体池框选它们，右击选择 **生成代理媒体**。

:::important
工作文件夹的设置是**保存在数据库工程**里的。这意味着如果别的成员连上了这个工程，达芬奇默认会去他电脑的 `E:\` 盘找缓存。如果他没 E 盘，软件直接报错。 **请务必在团队内部立下要求：任何人第一次在自己电脑上打开云端工程，第一件事就是去项目设置里，把这几个工作文件夹路径改成他们自己电脑上的本地 SSD！
:::
