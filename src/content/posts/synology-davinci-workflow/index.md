...
首先需要在 NAS 上创建整个视频项目的目录。博主选择将其创建在群晖的共享文件夹下，路径为：`/volume1/A1.公共事务/2026宣传片`。
...
**无论是哪种情况，使用了“非基准路径”的成员，都需要在自己的达芬奇里设置映射，让软件学会自动“翻译”路径。**
...
![img_1781954850100.png](./img_1781954850100.png)
...
:::caution[分享一些博主踩坑实录]
1. **子目录：** 挂载卷的路径里，宿主机路径末尾一定要加上 `/pg_data`，然后在部署 docker 前手动创建这个空文件夹。否则群晖的隐藏文件（如 `@eaDir` 或回收站）会导致 Postgres 认为目录非空，从而触发 `initdb` 失败并无限重启。
2. **密码初始化：** `POSTGRES_PASSWORD` 和 `POSTGRES_DB` 只有在 `pg_data` 完全为空的第一次启动时才会写入系统。如果你中途想改密码或数据库名，直接改 YAML 重启是**无效的**。你必须：改 YAML -> 停止容器 -> 删除容器 -> **清空宿主机的 `pg_data` 目录** -> 重新 `docker-compose up -d`。
:::

#### 2.2.3 使用达芬奇连接

打开达芬奇，在项目管理器的左上方点击**网络**，点击左栏下方的**添加项目库**，选**连接**：

- **名称:** `NAS_DaVinci` （必须与 YAML 中的 `POSTGRES_DB` 完全一致！）
- **位置:** 你的 NAS IP，这里就是`10.4.7.2` （注意：只支持纯 IP或纯域名，**绝对不能加 `:5432` 端口号**，否则必报未知主机错误！）
- **用户名:** 你设置的用户名，默认`postgres`
- **密码:** 你设置的密码。

![img_1781952670067.png](./img_1781952670067.png)

点击连接。如果此时瞬间连上，恭喜你，你的达芬奇协作服务器已经竣工了！如果连不上，一般达芬奇给的报错会比较笼统，这时可回到群晖的容器管理器，查看容器日志排错。
![img_1781952782172.png](./img_1781952782172.png)

## 3. 新建项目和代理位置修改

数据库通了，不代表可以直接剪了。直接在千兆网下剪辑 4K 或高码率 H.264 素材依然可能会卡成 PPT。我们需要配置“读写分离”，**使用代理剪辑并将代理片段本地化**。

### 3.1 新建项目

在达芬奇的网络项目库中，右键新建名为 `2026宣传片` 的项目。这部分操作和本地完全一样，但注意，导入素材时，**你的“媒体位置”必须选择我们第一章创建的那个 NAS 上的网络共享文件夹**。
![img_1781952940142.png](./img_1781952940142.png)

### 3.2 设置代理与缓存到本地

除非你的 NAS 和电脑之间有极其稳定的 5Gbps 甚至万兆（10GbE）网络连接，否则**博主强烈建议在项目中启用代理剪辑**，并强制将渲染缓存保存在本地固态硬盘。

*代理剪辑的原理很简单：达芬奇会在后台把 NAS 上难解码、码率高的高画质原片，转码成体积小、极易解码的低画质版本（如 DNxHR LB）。剪辑时看低画质代理，丝滑无比；最终导出时，达芬奇会自动调取 NAS 上的原片进行高画质渲染。*

**配置步骤：**

1. 每位成员在自己的达芬奇菜单栏点击`DaVinci Resolve -> 偏好设置`，进入 `系统 -> 媒体存储`。确保这个列表中的**第一个媒体存储位置**，是自己电脑上的本地视频项目库目录（如博主的`E:\视频工程`）。注意：达芬奇的缓存相对路径永远只认列表里的第一行！![img_1781954280157.png](./img_1781954280157.png)
2. 打开本地的达芬奇视频项目库（如博主的 `E:\视频工程`）新建 `ProxyMedia` 、`CacheClip` 和 `.gallery` 文件夹，分别保存代理、缓存、画廊静帧。有就不用再创建了。
3. 在达芬奇中打开新创建的项目，点击右下角齿轮进入 `项目设置  -> 主设置`。向下滚动找到 **工作文件夹**。将“代理生成位置”“缓存文件位置”和“画廊静帧位置”分别手动清空输入框，直接敲入刚才创建的相对文件夹名。![img_1781954165457.png](./img_1781954165457.png)
4. 在顶部菜单栏点击`播放 -> 代理的处理方式 -> 勾选 优先使用代理`。![img_1781953290850.png](./img_1781953290850.png)
5. 导入 NAS 视频后，在媒体池框选它们，右击选择 **生成代理媒体**。

:::important
这样设置后，数据库里保存的就只是“相对指令”。当博主打开工程时，达芬奇会自动去博主本地“媒体存储”的第一行路径下找，将缓存写进 `E:\视频工程\CacheClip`；当队友打开工程时，达芬奇会自动把缓存写进他本地设置的第一行路径（比如 `D:\缓存\CacheClip`）中。解耦本地物理路径与数据库设置，互不干扰！
:::

## 总结

一套“元数据上云、原片在 NAS、代理在本地”的高效剪辑流就此打通。这套流程解决了个人硬盘空间不足的痛点，同时让多人协作调色、加特效成为了现实。接下来，博主就要开始策划、拍摄、剪片了，尽请期待吧！