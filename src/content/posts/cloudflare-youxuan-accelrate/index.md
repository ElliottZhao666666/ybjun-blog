---
title: Cloudflare 全家桶站点国内优选提速！拒绝复杂方法，不用SaaS！
...
> 🎉 **【基础设施升级公告】** > 自 2026 年 5 月 10 日起，月半菌的 Blog 已全面完成 Cloudflare 边缘节点优选 IP 接入！本次底层架构改造大幅降低了跨洋路由丢包率，中国大陆地区访问体验已实现“秒开”级跨越。

熟悉我的朋友都知道，月半菌的个人博客是一个彻底的“Cloudflare 全家桶”拥趸。主站前端采用 Astro 框架并结合 GitHub 仓库实时部署至 Pages；后端则手搓了相册、评论、统计等多个 Worker API，底层对接 D1 关系型数据库与 R2 对象存储桶。

这套 Serverless 架构在工程化上极其优雅，但唯独存在一个致命痛点：**Cloudflare 免费版的 Anycast（泛播）网络对中国大陆极度不友好。** 默认分配的 IP 往往需要绕道美国西海岸，走拥堵的 163 国际骨干网，导致国内访客经常要看着 Loading 动画转上将近一分钟才能刷出内容。

针对这个问题，圈内最成熟的解法是使用“优选 IP / CNAME 优选”。但目前网上的教程大多针对纯前端或单一反代场景，常常需要引入 Cloudflare for SaaS 进行复杂的跨域接管，稍有不慎就会遇到各种 1000、522 报错，让人云里雾里。

本文不搞花里胡哨的骚操作，不卡 Bug，针对目前最新的 CF 托管方案，深度分享博主亲测的**原生同 Zone 优选全流程与踩坑实录**。

## 1. 优选节点究竟是什么？

要搞懂优选，必须先拆解 Cloudflare 那朵“小黄云（代理状态）”的底层逻辑。当小黄云开启时，CF 实际上在做两件事：

1. **解析层**：根据访客的网络环境，DNS 返回一个 CF 的泛播 IP（对于大陆用户，通常是拥堵的“减速” IP）。
