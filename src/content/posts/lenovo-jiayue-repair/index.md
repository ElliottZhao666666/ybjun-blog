...
![img_1786465641528.png](./img_1786465641528.png)
...
为什么博主会在一坤年后的今天，突然想起并深入研究这个问题？是因为最近几天，这玩意又有新情况了。

### 4.1 症状表现

前几天，亲戚发来求助，电脑开机突然进不去系统了。视频通话可以看到，**画面停留在 PXE 启动界面**。重启后虽然进了 Win7，但每次开机都会强制弹出蓝屏白字的 autochk（[看这里了解更多](https://www.ybjun.com/posts/windows-native-autochk)）。更要命的是，进入登录界面后，系统弹窗报错 edge 相关组件文件丢失。

这套连环症状，让博主立刻产生了一种不祥的预感：这块 18 岁高龄的 160G 主硬盘，是不是终于要寿终正寝了？或者说至少现在，已经开始出现了间歇性掉盘。

### 4.2 问题分析

#### 4.2.1 硬盘本身的健康状况

首先当然是检查硬盘 SMART 信息。用 CrystalDiskInfo 读取老硬盘的完整 SMART 数据，结果有点出乎意料 —— **这块十八年的老轱辘棒子，状态竟然还不错！**

![img_1786466419410.png](./img_1786466419410.png)

* `05`（重映射扇区计数）：当前 100，最差 100，数据 0，状态良好
* `C5`（当前待映射扇区） + `C6` （不可校正扇区数）：数据都是 0！
* `C7`（UltraDMA CRC 错误计数）：数据 0，传输错误为零！
* `09`（通电时间） + `0C` （通电次数）：4887 小时，约合 203 天，通电 3463 次。要知道，机龄已经十八年了呀！

:::tip[读取错误率那么高是何意味？]
大家可能会在看到 `01`、`07`、`C3` 动辄上亿的原始值时直接吓一跳，但这是希捷特有的 `48-bit LBA` 寄存器记录机制：
**`01` 读取错误率 & `C3` 硬件 ECC 校正计数 (原始值均为 134343980)：**
注意看，这两个原始值是完全相等的。在希捷的规则里，底层的 32 位记录的是 “总读取操作数”，高 16 位记录的才是 “真正的读取错误数”。这两个值完全相等，意味着这 1.34 亿次的底层读取中，所有的误码都被硬盘主控的硬件 ECC（错误检查和纠正）算法完美修正了。只要当前值（98）远大于临界值（6），这就代表读取极其健康。
**`07` 寻道错误率 (原始值 249567171)：** 
同理，记录的是总寻道次数，当前值（84）远大于临界值（30），寻道马达工作完美。
:::

不过，我们也不能完全通过这么好的 SMART 数据就断言硬盘一定没问题，机械硬盘运转十八年，即使没有出现坏道，也必然存在老化。比如：
1. 磁头定位精度下降：音圈电机老化，寻道时间变长，偶尔出现读写出错后重试成功的情况，SMART 不会记录这种 "最终成功" 的错误。
2. 主轴电机轴承磨损：导致启动时转速稳定时间变长，冷启动容易掉盘。
3. 磁碟磁粉衰减：部分区域的磁信号变弱，读取需要多次重试，表现为系统卡顿。
4. 电路板元件老化：电容、电阻的参数漂移，导致供电不稳、信号质量下降。

#### 4.2.2 电源问题可能才是主因

既然硬盘没坏，为什么会频繁掉盘？真相的另一头，指向了机箱里那颗同样服役了 18 年的**航嘉 HK280-22GP** (额定 180W) 电源。
![img_1786467608335.png](./img_1786467608335.png)

且不说这颗电源功率小、用料肯定也一般，经过近 20 年的消耗，电源内部的滤波和稳压电解电容必然会存在鼓包、漏液、容量衰减。

硬盘启动瞬间的电流是很大的。+12V 供电瞬间电流可以达到 2A 以上。如果电源 +12V 输出能力下降，启动时电压跌落过多，就可能导致硬盘初始化失败，影响硬盘健康。具体表现为：
1. **冷启动掉盘。** 第一次开机找不到硬盘，重启后因为硬盘还在旋转、启动电流小，反而能识别。
2. **间歇性掉盘。** 系统运行中，电源负载波动导致电压瞬时跌落，硬盘短暂失联后又恢复，系统可能会磁盘检查。
3. **文件损坏。** 写入过程中掉盘，导致文件系统元数据损坏。

**启动 PXE 的现象也很符合这个推断** —— BIOS 自检时硬盘供电不稳、没转起来，没有就绪，于是 BIOS 按启动顺序跳过主硬盘（老硬盘），再加上新硬盘作为数据盘没有可引导的活动主分区、没有 U 盘、光驱被拔了，因此 BIOS 只能尝试最后的启动设备 —— 网络 PXE。

**博主认为更值得注意的是，电源老化是系统性问题。它不仅影响硬盘，也影响主板、内存、显卡。供电纹波变大后，整个系统的稳定性都会下降，未来很有可能出现其它各种莫名其妙的故障。**

#### 4.2.3 完整推演：一次 "掉盘" 的全过程
...
sequenceDiagram
    ...
    用户->>电源: 按下开机键
    电源->>硬盘: 开始供电，电压爬升
    Note over 硬盘: 主轴电机启动，电流峰值
    电源-->>硬盘: 电压跌落，低于阈值
    Note over 硬盘: 初始化失败，处于半就绪状态
    
    BIOS->>硬盘: 发送IDENTIFY命令
    硬盘-->>BIOS: 勉强响应，返回型号
    Note over BIOS: 识别到硬盘，继续自检
    
    BIOS->>硬盘: 读取MBR引导扇区
    Note over 硬盘: 磁头寻道，电流再次波动
    硬盘-->>BIOS: 读取成功（简单读取）
    
    系统内核->>硬盘: 加载驱动，发送多个扩展命令
    Note over 硬盘: 复杂命令交互 + IDE模拟层转换
    Note over 电源: 负载增加，电压进一步不稳
    硬盘--x系统内核: 传输CRC错误 / 命令超时
    Note over 系统内核: 磁盘IO异常，启动卡死 / 掉盘
```

同时，结合第三章的分析，我们可以得出一个很无奈的结论：

**这台电脑已经几乎无法使用 2011 年以后生产的任何硬盘作为系统盘。因为这个时间点以后的硬盘都支持了 `4K AF`。**

VT8237A 南桥的 IDE 模拟层兼容性，就是一道无法逾越的技术鸿沟。我们可以找同期的二手老硬盘来换，但那也是用一个快挂的换另一个快挂的。SATA 固态？更不用想了 ——SSD 对 SATA 协议的依赖更深，高级命令更多，兼容性问题只会更严重。

再说电源，全新的 ATX 电源不难找，但匹配品牌机机箱的非标电源、功率大于 180W 的、甚至可能需要同期且状态良好的 ATX 电源，就可遇不可求了。

所以，这块老硬盘暴毙的那天，或者电源彻底挂掉的那天，也应该就是这台电脑正式退休的日子了。

## 5 最后的处置方案

分析到这里，答案其实已经很明确了。**对于一台十八岁的老电脑，再投入金钱和精力去 "修复" 抑或是 "升级" 已经毫无价值。**

基于此，博主已经向亲戚交代了最终的处置方案，鉴于偶尔还要“迁就”使用，所以：

1. 把 C 盘和 D 盘里所有重要的文档、照片、资料，通过 U 盘、移动硬盘或者网盘全部备份出来。备份是第一位的，数据比机器值钱。
2. 以后重要文件一律存到 E 盘和 F 盘（新硬盘），C 盘 D 盘只放系统和软件，不再存放任何有价值的数据，包括临时文件。桌面和文档路径也转移到新硬盘上。
3. 接受现实。这台机器能用一天算一天，不要指望它还能再战斗多长时间。哪天彻底开不了机了，就是寿终正寝了，不用觉得可惜。
![img_1786468930181.png](./img_1786468930181.png)

## 6 一点题外话：技术代际的残酷

这台家悦电脑的故事，其实也是整个 PC 硬件发展史的一个缩影。

我们常说 "向下兼容" 是 PC 平台的优良传统，但**兼容性从来不是无限的。** SATA 替代 IDE 用了十年，中间有无数的过渡方案、模拟层、兼容模式；但当时间跨度拉到十八年，代际的鸿沟终究无法跨越。

VT8237A 南桥设计的时候，SATA 1.0 才刚普及，设计者不可能预见十几年后 SATA 3.0 硬盘的种种新特性，更不可能去实现 NVMe 等更新的技术。同样，现在的硬盘厂商也不会去测试一款早已停产的古董南桥是否兼容。这不是谁的错，这就是技术进步的代价 —— **新的更好、更快、更强，但也不再回头等那些老家伙。**

从 160GB 到 500GB，看似只是容量数字的变化，背后却是整整几代存储技术的迭代。而那块不起眼的 VT8237 系列南桥，就像一道时间的闸门，一边是 2008，一边是 2026，闸门两端的硬件，说着相似却又不通的语言。

**这台联想家悦还能坚持多久，没人知道。但至少在它彻底退休之前，我们搞懂了它身上那个困扰许久的谜题 —— 对于喜欢折腾硬件的人来说，这本身就足够有意思了。**