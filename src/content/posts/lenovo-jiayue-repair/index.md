...
当新硬盘作为系统盘时，BIOS 笨拙地按照 512 字节的步长去读取。当跨越 4K 山区物理边界时，老 BIOS 并没有相应的地址转换和时序等待逻辑，导致读取数据出现了严重的字节偏移。
这就导致了荒谬的一幕：硬盘里的数据是正确的，**但 BIOS 读进内存里的数据全是乱码。** 当 BIOS 把读错的启动画面位图数据塞给显卡渲染时，直接导致了震撼的“花屏死机”。