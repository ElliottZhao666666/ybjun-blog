...
其实，产生在这个阶段运行自己程序的想法，源于博主在十几年前试用“360系统重装大师”的记忆。当时只下载了不到200MB的差异文件，接着重启在这个界面刷了一会英文字符，就替换完了，但这个界面显示一个很大的“360SAFE”，印象特别深。

![img_1790132988435.png](./img_1790132988435.png)

:::note
网图，图源见水印。

关于系统重装大师在这里显示 logo 的实现，这是正在进一步研究的内容，但因为实在找不到可用的上古版本360重装大师（至少2015年以后的重装方案就已经全改成 sysprep 了，博主印象里这种老实现应该是 2013-2014 年间的版本），所以目前只能全靠推测。比如可以用一个完全重写的简易驱动接管 bootvid.dll ，直接操作显存，往绝对位置里面写位图像素（这个模式固定 640x480 ，绝对位置也没问题）。
:::
...
- **没有 C 标准库**：我们熟悉的 `printf` 打印、`fopen` 读写文件在这里统统失效。就连读取键盘输入也只能截获最底层的数据包。
...
其实，我们在使用Windows时常见的**可执行程序文件`*.exe`**、**动态链接库文件`*.dll`**、**驱动文件`*.sys`**、甚至还有**UEFI启动文件`*.efi`**、**Windows 多语言资源文件`*.mui`**、**屏幕保护程序`*.scr`**、**Windows 主题样式`*.msstyles`** 等等，在二进制结构上**都没有任何区别！** 它们统统都是 **PE（Portable Executable）文件格式**。
...
这套基于 32 位 x86 架构，为 XP 平台写出的上古 Native 代码，如果**扔进最新的现代 Windows** 里会怎样？博主进行了两组实验。
...
但在画面上，出现了一个极其诡异的现象：我们的文本中间，**凭空出现了一个纯黑的方形“黑洞”**，直接吃掉了部分内容！

这个黑洞，正是**主板开机 Logo** 原本所在的位置！ 看来无论是 UEFI 启动从固件 BGRT（启动图形资源表）中读取厂家 Logo，还是传统 BIOS 启动读取 Windows 徽标，微软从 Windows 8 开始贯彻了 **“无缝启动（Seamless Boot）”** 的理念。

系统底层的图形引擎在帧缓存（Framebuffer）中围绕 Logo 划定了一个区域，这块区域储存Logo信息并被严格保护了起来。当我们的 `NtDisplayString` 试图渲染文字时，只要像素落入这个区域，系统就会**直接将渲染指令强行丢弃**。宁愿让画面像被抠图一样残缺，也绝对不允许任何底层文本污染开机 Logo，破坏普通用户的体验。不过，本来这个阶段的原始程序和显示内容早已被弃用，**已经合并入了启动画面**，所以这么做当然是可以的。只是这又向我们秀了一波 Windows 的向下兼容性究竟有多“变态”！

## 结语

抛弃高级语言的温室，在最底层的限制和荒野中摸爬滚打，我们才能真正看清操作系统的运行脉络。那个伴随我们童年的蓝色扫盘界面，不仅是一段回忆，更是通向 Windows 内核深处的一扇门。希望大家能从本文中有所学习，对 Windows 内核的认知更上一层楼！